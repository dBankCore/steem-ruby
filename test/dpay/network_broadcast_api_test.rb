...
module DPay
  class NetworkBroadcastApiTest < DPay::Test
    def setup
      @api = DPay::NetworkBroadcastApi.new(url: TEST_NODE)
      @jsonrpc = Jsonrpc.new(url: TEST_NODE)
      @methods = @jsonrpc.get_api_methods[@api.class.api_name]
    end
    def test_api_class_name
      assert_equal 'NetworkBroadcastApi', DPay::NetworkBroadcastApi::api_class_name
    end

    def test_inspect
      assert_equal "#<NetworkBroadcastApi [@chain=dpay, @methods=<2 elements>]>", @api.inspect
    end
